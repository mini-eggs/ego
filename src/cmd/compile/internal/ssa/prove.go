...
// pairings of variables (v, w). Without a priori knowledge the
...
// domain represents the domain of a variable pairing in which a set
// of relations is known. For example, relations learned for unsigned
// pairings cannot be transferred to signed pairings because the same bit
...
type pairing struct {
	v, w *Value // a pairing of values, ordered by ID.
	// v can be nil, to mean the zero value.
	// for booleans the zero value (v == nil) is false.
	d domain
}

// fact is a pairing plus a relation for that pairing.
type fact struct {
	p pairing
	r relation
}
...
// factsTable keeps track of relations between pairings of values.
//
...
type factsTable struct {
	...
	facts map[pairing]relation // current known set of relation
...
func newFactsTable(f *Func) *factsTable {
	...
	ft.facts = make(map[pairing]relation)
...
//
// By far, the most common redundant pairing are generated by bounds checking.
